...
class CreateUser(email.ActivationEmail):
    ...
    def get_context_data(self):
        ...
        context.update({
            ...
            'support_email': "ebirutech@gmail.com",
            'app_name': "Fiador",
...
class CustomForgotEmail(email.UsernameResetEmail):
    template_name = "email/forgot_email/body.html"
    ...
    def get_context_data(self):
        ...
        context.update({
            ...
            "confirm_url": self.context.get("confirm_url", ""),
        })
        return context
    ...
    def send(self, to, *args, **kwargs):
        ...
        subject = render_to_string('email/forgot_email/subject.txt', context).strip()
        body_html = render_to_string(self.template_name, context)
        body_text = render_to_string('email/forgot_email/body.html', context)
...
class CustomEmailReset(email.ActivationEmail):
    ...
    def get_context_data(self):
        ...
        print(f"URL generada: {context.get('activation_url', 'No encontrada')}")
        context = {
            'site_name': "Fiador",
            'domain': settings.DOMAIN,
            'protocol': settings.PROTOCOL,
            ...
            'activation_url': f"{settings.PROTOCOL}://{settings.DOMAIN}/activate/new-email/{self.context.get('uid', '')}/{self.context.get('token', '')}"
        }
        return context
